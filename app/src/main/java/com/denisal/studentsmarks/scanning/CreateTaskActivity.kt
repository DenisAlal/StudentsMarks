...
class CreateTaskActivity : AppCompatActivity(), AdapterView.OnItemSelectedListener {
        ...
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            //binding.goScan.isEnabled = false
        ...
        private fun init() {
            ...
            goScan.setOnClickListener{
                ...
                if (nameTask.text.isNotEmpty()) {
                    Log.e("lo11", getCourses.toString())
                    for (index in getCourses.indices) {
                        ...
                        if (value.courseId == subjArray[position].id
                            ...
                            if (lesCheck == value.name) {
                                val check = insertToDB.insertTask(nameTask.text.toString(), value.courseId, value.id)
                                if (check) {
                                    nameTask.text = ""
                    ...
                    Toast.makeText(applicationContext, "Введите название задания", Toast.LENGTH_LONG).show()
                }
        ...
        private fun getDate() {
            ...
            for (index in getCourses.indices) {
                ...
                if (value.name.contains(lesson.selectedItem.toString())) {
                    choiceLesson.add(LessonData(value.id,value.name,value.date, value.time,
